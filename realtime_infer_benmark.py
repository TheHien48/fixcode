...
def main():
    ...
    is_recording = False
    start_time = 0.0
    latency_records = [] # Lưu danh sách đo lường
    ...
    print("   2. Múa ký hiệu/câu hoàn chỉnh.")
    print("   3. Hệ thống sẽ tự động đo thời gian đến lúc chốt câu.")
    ...
    with mp_hands.Hands(
        ...
        while cap.isOpened():
                        ...
                        if len(gesture_seq_buffer) >= 1:
                            ...
                            if s_conf > SENTENCE_THRESHOLD and sid != 0:
                                ...
                                if is_recording:
                                    ...
                                    is_recording = False # Reset trạng thái
            else:
                gname = "(không có)"

            # --- D. HIỂN THỊ KẾT QUẢ TRÊN MÀN HÌNH ---
            h, w = frame.shape[:2]
            cv2.rectangle(frame, (0, 0), (w, 150), (0, 0, 0), -1) # Nới rộng khung đen
            ...
            # --- HIỂN THỊ TRẠNG THÁI ĐO E2E LÊN CAMERA ---
            if is_recording:
                frame_rgb = put_vietnamese_text(frame_rgb, "[ĐANG ĐO LATENCY - Đợi chốt câu...]", (10, 120), font_size=20, color=(255, 0, 0))
            else:
