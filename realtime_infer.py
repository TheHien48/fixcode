...
def main():
    # Thêm flush=True để ép Python in log ngay lập tức, không bị ngậm buffer khi crash
    print("="*60, flush=True)
    print("🚀 HỆ THỐNG SIGN2VI - CHẾ ĐỘ VƯỢT LỖI EGL TỐI THƯỢNG V2 🚀", flush=True)
    print("="*60, flush=True)
    ...
    print("✅ Đang nạp MediaPipe ĐẦU TIÊN để đoạt EGL...", flush=True)
    
    # 1. KHỞI TẠO MEDIAPIPE TRƯỚC TIÊN
    with mp_hands.Hands(
        ...
        print("✅ MediaPipe đã nuốt trọn EGL an toàn! Không sợ crash nữa.", flush=True)
        
        # 2. KHỞI TẠO ONNX NGAY SAU KHI MEDIAPIPE ĐÃ CHIẾM CHỖ
        try:
            print("⏳ Đang load ONNX Models...", flush=True)
            ...
            print("✅ Load ONNX thành công!", flush=True)
        except Exception as e:
            print(f"❌ Lỗi khi load ONNX: {e}", flush=True)
            return
            
        # 3. MỞ CAMERA CUỐI CÙNG
        ...
        print("💡 Hệ thống đang quét ngầm. Múa tay đi, kết quả sẽ in ra đây!", flush=True)
