...
def main():
    print("="*60, flush=True)
    print("🚀 HỆ THỐNG SIGN2VI - CHẾ ĐỘ BYPASS BỘ NHỚ TĨNH 🚀", flush=True)
    print("="*60, flush=True)
    ...
    # 1. KÍCH HOẠT MEDIAPIPE TRƯỚC (BẰNG MA THUẬT STATIC_IMAGE_MODE)
    print("⏳ [1/3] Kích hoạt MediaPipe (Static Mode)...", flush=True)
    mp_hands = mp.solutions.hands
    hands = mp_hands.Hands(
        static_image_mode=True,  # QUAN TRỌNG NHẤT: Bỏ chế độ video tracking nặng nề
        max_num_hands=2,
        min_detection_confidence=0.7
    )
    print("✅ MediaPipe an toàn, đã né luồng đồ họa chết!", flush=True)
    ...
    # 3. KẾT NỐI CAMERA
    print("⏳ [3/3] Kết nối Camera...", flush=True)
    ...
    print("\n💡 HỆ THỐNG ĐÃ SẴN SÀNG! Múa đi, kết quả in thẳng ra màn hình!", flush=True)
