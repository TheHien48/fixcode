...
print("="*60, flush=True)
print("🚀 HỆ THỐNG SIGN2VI - CÚ HACK KERNEL XInitThreads 🚀", flush=True)
print("="*60, flush=True)

# =========================================================================
# BƯỚC 1: TIÊM LỆNH XInitThreads VÀO NHÂN LINUX BẰNG CTYPES (CÚ CHỐT)
# Ép Ubuntu cho phép MediaPipe xài đa luồng mà không bị Segfault!
# =========================================================================
print("⏳ [1/4] Mở khóa đa luồng X11 cấp thấp...", flush=True)
import ctypes
try:
    x11 = ctypes.CDLL('libX11.so.6')
    x11.XInitThreads()
    print("✅ Đã tiêm XInitThreads! Cầu dao X11 đã được bọc thép.", flush=True)
except Exception as e:
    print(f"⚠️ Không thể gọi XInitThreads: {e}", flush=True)

# =========================================================================
# BƯỚC 2: GỌI MEDIAPIPE (Lúc này X11 đã ngoan ngoãn chấp nhận đa luồng)
# =========================================================================
print("⏳ [2/4] Kích hoạt MediaPipe...", flush=True)
...
print("✅ MediaPipe khởi tạo thành công mỹ mãn!", flush=True)

# =========================================================================
# BƯỚC 3: NẠP OPENCV HEADLESS VÀ ONNX
# =========================================================================
print("⏳ [3/4] Nạp AI và OpenCV...", flush=True)
import cv2
...
def main():
    ...
    print("⏳ [4/4] Bật mô hình ONNX và kết nối Camera...", flush=True)
    try:
    ...
    print("\n💡 THẮNG BẠI TẠI KỸ NĂNG! Đưa tay lên múa cái ký hiệu Cảm Ơn để chốt hạ đồ án nào!", flush=True)
