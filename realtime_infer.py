...
print("="*60, flush=True)
print("🚀 HỆ THỐNG SIGN2VI - CHẾ ĐỘ THUẦN CPU (CHỐNG SẬP EGL) 🚀", flush=True)
print("="*60, flush=True)

# Nạp OpenCV Headless / tiêu chuẩn trước
import cv2
cv2.ocl.setUseOpenCL(False)
cv2.setNumThreads(1)

print("⏳ [1/2] Khởi tạo MediaPipe (Chế độ thuần CPU)...", flush=True)
import mediapipe as mp
mp_hands = mp.solutions.hands

# Ép MediaPipe chạy chế độ CPU thuần túy, tuyệt đối không cho gọi EGL phần cứng
hands = mp_hands.Hands(
    static_image_mode=False, 
    max_num_hands=2,
    model_complexity=0, # Dùng model nhẹ nhất, bắt buộc chạy trên CPU
    min_detection_confidence=0.5,
    min_tracking_confidence=0.5
)
print("✅ MediaPipe đã khởi động thành công trên CPU!", flush=True)

print("⏳ [2/2] Nạp mô hình ONNX...", flush=True)
...
def main():
    ...
    print("\n💡 HỆ THỐNG ĐÃ SẴN SÀNG! Đưa tay lên test ngay đi mày!", flush=True)
