...
print("="*60, flush=True)
print("🚀 HỆ THỐNG SIGN2VI - ÉP LUỒNG KHỞI TẠO EGL TUYỆT ĐỐI 🚀", flush=True)
print("="*60, flush=True)

# ====================================================================
# 1. ĐOẠT QUYỀN EGL NGAY LẬP TỨC TRƯỚC KHI BẤT CỨ THỨ GÌ KHÁC NẠP VÀO
# ====================================================================
print("⏳ [1/4] Nạp MediaPipe...", flush=True)
import mediapipe as mp
mp_hands = mp.solutions.hands

print("⏳ [2/4] Kích hoạt EGL trực tiếp (Chiếm quyền đồ họa)...", flush=True)
# Khởi tạo Hands NGAY LẬP TỨC ở global để EGL được chốt trước khi cv2 nạp
...
print("✅ EGL đã an toàn! Không còn khe hở để xung đột.", flush=True)

# ====================================================================
# 2. BÂY GIỜ MỚI CHO PHÉP OPENCV VÀ ONNX XUẤT HIỆN
# ====================================================================
print("⏳ [3/4] Nạp OpenCV (Khóa nạp libGL muộn)...", flush=True)
import cv2
cv2.ocl.setUseOpenCL(False)
cv2.setNumThreads(1)

print("⏳ [4/4] Nạp ONNX Runtime...", flush=True)
import onnxruntime as ort

print("✅ Nạp thư viện hoàn tất, sẵn sàng múa!", flush=True)
...
def main():
    ...
    print("🎥 Đang kết nối Camera USB...", flush=True)
    cap = cv2.VideoCapture(0, cv2.CAP_V4L2)
    if not cap.isOpened():
        print("❌ LỖI: Camera đang bị kẹt!", flush=True)
        return

    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
    print("💡 Hệ thống đang chạy. Đưa tay lên múa đi, kết quả sẽ in ra đây!", flush=True)
    print("💡 Bấm Ctrl + C để dừng phần mềm.\n", flush=True)
    ...
    try:
        while cap.isOpened():
            ...
            # Sử dụng đối tượng hands đã được khởi tạo tít ở trên cùng
                    ...
                    if current_time > lock_until:
                        ...
                        if len(gesture_seq_buffer) >= 1:
                            ...
                            if s_conf > SENTENCE_THRESHOLD and sid != 0:
                                ...
                                print(f"🎯 KẾT QUẢ CHỐT CÂU: {final_sentence} (Từ đang nhận: {gname})", flush=True)
