...
# BẮT BUỘC NẠP OPENCV TRƯỚC ĐỂ MỞ CAMERA
import cv2
...
def main():
    print("="*60, flush=True)
    print("🚀 HỆ THỐNG SIGN2VI - CHIÊU THỨC ÉP XUNG CAMERA 🚀", flush=True)
    print("="*60, flush=True)
    ...
    # 1. KẾT NỐI CAMERA ĐẦU TIÊN ĐỂ KHÓA CHẶT ĐỒ HỌA TEGRA
    print("⏳ [1/3] Đánh thức Camera và chiếm luồng EGL...", flush=True)
    ...
    # Đọc nháp 5 frame để ép hệ thống khởi tạo xong toàn bộ driver đồ họa
    for _ in range(5):
        cap.read()
    print("✅ Camera đã ổn định 100%! Đồ họa đã bị khóa.", flush=True)

    # 2. SAU KHI CAMERA ỔN ĐỊNH MỚI NẠP MEDIAPIPE (TRÌ HOÃN NẠP)
    print("⏳ [2/3] Nạp MediaPipe ngầm (xài ké EGL của Camera)...", flush=True)
    ...
    print("✅ MediaPipe đã nạp thành công, không còn cơ hội đụng độ!", flush=True)
    ...
    print("\n💡 HỆ THỐNG SẴN SÀNG! Đưa ký hiệu lên test đi mày!", flush=True)
