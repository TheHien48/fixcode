...
COOLDOWN_TIME = 2.5         # Đóng băng 2.5 giây sau khi chốt được câu
...
def main():
    ...
    gname = "(không có)"
    lock_until = 0.0
    final_sentence = "(đang chờ múa...)"

    print("🎥 Đang chạy nhận diện ngầm (Không bật cửa sổ GUI để tránh lỗi Jetson)...")
    print("💡 Bấm Ctrl + C trong Terminal để dừng chương trình bất cứ lúc nào.")
    
    # 3. KHỞI TẠO MEDIAPIPE (Đã thêm model_complexity=0 để chống lỗi EGL)
    with mp_hands.Hands(
        static_image_mode=False,
        max_num_hands=2,
        model_complexity=0,  # THÊM DÒNG NÀY: ÉP DÙNG MODEL NHẸ NHẤT
        ...
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                    ...
                    if current_time > lock_until:
                        ...
                        if len(gesture_seq_buffer) >= 1:
                            ...
                            if s_conf > SENTENCE_THRESHOLD and sid != 0:
                                ...
                                print(f"🎯 CHỐT CÂU: {final_sentence} (Gesture đang nhận: {gname})")
                ...
                gname = "(không có)"
                if frame_count % 30 == 0:
                    print("⏳ Đang quét... (Không thấy tay)")
            ...
            # Nếu vẫn bị văng ở bước hiển thị, mày có thể comment dòng imshow dưới đây lại
