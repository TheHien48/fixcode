...
print("="*60, flush=True)
print("🚀 HỆ THỐNG SIGN2VI - CHỐNG XUNG ĐỘT LÕI C++ TUYỆT ĐỐI 🚀", flush=True)
print("="*60, flush=True)

# =========================================================================
# BƯỚC SINH TỬ 1: CHỈ ĐƯỢC PHÉP NẠP VÀ KHỞI TẠO MEDIAPIPE TẠI ĐÂY
# =========================================================================
print("⏳ [1/3] Kích hoạt MediaPipe (Độc chiếm lõi Protobuf)...", flush=True)
import mediapipe as mp
mp_hands = mp.solutions.hands

# Khởi tạo ngay lập tức để MediaPipe chốt chặt vùng nhớ đồ họa và C++
hands = mp_hands.Hands(
    static_image_mode=True, 
    max_num_hands=2,
    min_detection_confidence=0.7
)
print("✅ MediaPipe đã khóa EGL và Protobuf an toàn tuyệt đối!", flush=True)

# =========================================================================
# BƯỚC SINH TỬ 2: BÂY GIỜ MỚI ĐƯỢC PHÉP NẠP CÁC KẺ THÙ (ONNX, OPENCV)
# =========================================================================
print("⏳ [2/3] Nạp OpenCV và ONNX (Đứng xếp hàng chờ)...", flush=True)
import cv2
...
print("✅ Nạp xong thư viện phụ!", flush=True)
...
def main():
    ...
    print("⏳ [3/3] Nạp mô hình ONNX và khởi động Camera...", flush=True)
    try:
    ...
    print("\n💡 ĐỒ ÁN ĐÃ LÊN SÓNG! Múa ký hiệu Cảm Ơn để test ngay nào!", flush=True)
    ...
    try:
        while cap.isOpened():
            ...
            # Sử dụng objects `hands` đã được niêm phong ở đầu file
