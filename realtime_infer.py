...
# 1. TẨY TỦY: ĐUỔI CỔ OPENCV HỆ THỐNG BẰNG MỌI GIÁ
import sys
sys.path = [p for p in sys.path if 'dist-packages' not in p]

# 2. NẠP MEDIAPIPE ĐẦU TIÊN ĐỂ LÀM VUA CÁI PROTOBUF C++
import mediapipe as mp

# 3. NẠP ONNX THỨ HAI
import onnxruntime as ort

# 4. NẠP OPENCV HEADLESS (TRONG VENV) CUỐI CÙNG
import cv2
...
def main():
    print("="*60, flush=True)
    print("🚀 HỆ THỐNG SIGN2VI - CHẾ ĐỘ CÁCH LY TUYỆT ĐỐI 🚀", flush=True)
    print("="*60, flush=True)
    ...
    print("⏳ [1/3] Khởi tạo MediaPipe (Chế độ siêu tĩnh)...", flush=True)
    ...
    print("✅ MediaPipe đã sống sót! Không còn đụng độ EGL.", flush=True)

    print("⏳ [2/3] Nạp mô hình ONNX...", flush=True)
    try:
    ...
    print("⏳ [3/3] Bật Camera...", flush=True)
    ...
    print("\n💡 HỆ THỐNG ĐÃ LÊN SÓNG! Chuẩn bị múa cái ký hiệu cảm ơn đó cho hệ thống nhận diện đi!", flush=True)
