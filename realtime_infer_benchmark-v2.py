...
def main():
    print("="*50)
    print("🚀 KHỞI ĐỘNG HỆ THỐNG ĐO LATENCY E2E 🚀")
    print("="*50)
    ...
    print("💡 Hướng dẫn đo Benchmark:")
    print("   1. Đưa tay lên, nhấn phím 's' để BẮT ĐẦU đếm giờ.")
    print("   2. Múa tuần tự các ký hiệu.")
    print("   3. Hạ tay và nhấn phím SPACE (khoảng trắng) để chốt câu và đo thời gian.")
    ...
    with mp_hands.Hands(
        ...
        while cap.isOpened():
            ...
            # 1. Hàng đầu tiên: Ký hiệu AI (Ví dụ: BAN)
            ...
            if current_time > lock_until:
                for idx in gesture_seq_buffer:
                    ...
                    # 2. Hàng thứ hai: Dịch từ AI (BAN) sang tiếng Việt (bạn) để hiển thị trong mục Từ:
            ...
            # Hiển thị các từ tiếng Việt đã ghép
            frame_rgb = put_vietnamese_text(frame_rgb, f"Từ: {seq_str_vi}", (10, 45), font_size=24, color=(255, 255, 0))
            
            if current_time > lock_until and len(gesture_seq_buffer) == 0:
                final_sentence = "(đang chờ ...)"
                
            # 3. Hàng thứ ba: Câu chốt hoàn chỉnh
            frame_rgb = put_vietnamese_text(frame_rgb, f"Câu: {final_sentence}", (10, 85), font_size=28, color=(255, 255, 255))
            
            # Hiển thị trạng thái đo E2E
            if is_recording:
                frame_rgb = put_vietnamese_text(frame_rgb, "[ĐANG ĐO - Múa xong nhấn SPACE chốt câu]", (10, 120), font_size=20, color=(255, 0, 0))
            else:
                frame_rgb = put_vietnamese_text(frame_rgb, "[Nhấn 's' để bắt đầu bấm giờ]", (10, 120), font_size=20, color=(150, 150, 150))
            ...
            if key == ord('c'):
                ...
                if is_recording: 
                    print("[HỦY BỎ] Đã hủy lần đo hiện tại do reset chuỗi.")
                    is_recording = False
            ...
            if key == ord('s'):
                if not is_recording:
                    ...
                    print("\n[BẮT ĐẦU ĐO]... Hãy múa tay ngay lập tức!")

            # THÊM PHÍM SPACE ĐỂ CHỐT CÂU THỦ CÔNG
            if key == 32: 
                if len(gesture_seq_buffer) >= 1:
                    ...
                    # --- GHI NHẬN KẾT THÚC ĐO E2E LATENCY ---
                    if is_recording:
