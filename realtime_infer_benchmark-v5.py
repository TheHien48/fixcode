...
def main():
    print("="*50)
    print("🚀 KHỞI ĐỘNG HỆ THỐNG ĐO LATENCY E2E (TENSORRT) 🚀")
    print("="*50)
    ...
    try:
        # Ưu tiên TensorRT, nếu không được tự động lùi về CUDA hoặc CPU
        trt_providers = ['TensorrtExecutionProvider', 'CUDAExecutionProvider', 'CPUExecutionProvider']
        gesture_sess = ort.InferenceSession("models/gesture_classifier.onnx", providers=trt_providers)
        seq_sess = ort.InferenceSession("models/seq_model.onnx", providers=trt_providers)
    ...
    # Đổi tên file CSV để không ghi đè lên kết quả đo CPU lúc nãy
    csv_file = open("results/e2e_latency_log_trt.csv", mode='w', newline='', encoding='utf-8')
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(["ID Lần Thử", "Câu Dịch", "Latency E2E (ms)"])
    test_id = 1

    print("✅ Hệ thống đo E2E bằng TensorRT đã sẵn sàng!")
    print("💡 Hướng dẫn đo Benchmark (Theo Bảng 19):")
    ...
    with mp_hands.Hands(
        ...
        while cap.isOpened():
            ...
            frame_rgb = put_vietnamese_text(frame_rgb, "[Múa xong -> Bấm SPACE để đo Latency TensorRT]", (10, 120), font_size=20, color=(150, 150, 150))

            frame = cv2.cvtColor(frame_rgb, cv2.COLOR_RGB2BGR)
            cv2.imshow("Sign2Vi - Realtime E2E Benchmark (TensorRT)", frame)
    ...
    if latency_records:
        ...
        p50_lat = np.percentile(latency_records, 50)  
        p95_lat = np.percentile(latency_records, 95)
        print("\n" + "="*50)
        print("TỔNG KẾT ĐÁNH GIÁ END-TO-END LATENCY (TENSORRT)")
        print("="*50)
        ...
        print("Kết quả chi tiết đã lưu tại: results/e2e_latency_log_trt.csv")
